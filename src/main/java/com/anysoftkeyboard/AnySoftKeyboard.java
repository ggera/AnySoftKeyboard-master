...
public abstract class AnySoftKeyboard extends AnySoftKeyboardKeyboardSwitchedListener implements
        ...
        switch (primaryCode) {
            ...
            default:
                ...
                if (isWordSeparator(primaryCode)) {
                    if(key.label.hashCode()>=4961&&key.label.hashCode()<=4966)
                            primaryCode=(mInputView.isShifted())?key.popupCharacters.hashCode():key.label.hashCode();
                    ...
                    if (mControlKeyState.isActive() && primaryCode >= 32 && primaryCode < 127) {
                        ...
                        Log.d(TAG, "CONTROL state: Char was %d and now  serez it is %d", primaryCode, controlCode);
                        if (controlCode == 9) {
                            sendTab();
                        } else {
                    ...
                    else if((key.label.hashCode()>=4608 && key.label.hashCode()<=4951))
                    {
                        CharSequence label=(mInputView.isShifted()&&key.label.hashCode()<=4951&&key.popupCharacters==null)?key.popupCharacters:key.label;

                        if(key.label.hashCode()%8!=5 && ic.getTextBeforeCursor(1,0).hashCode()!=key.label.hashCode())
                        {


                            System.out.println("Text before Cursor"+ic.getTextBeforeCursor(1,0));
                            System.out.println("Popup Cursor"+key.popupCharacters);

                            ic.deleteSurroundingText(1, 0);
                            ic.commitText(label, 0);
                        ...
                        System.out.println("Serex grr %d"+primaryCode);
                    }
    ...
    public void onText(Key key, CharSequence text) {
        ...
        abortCorrection(true, false);
        if(key.codes[0]>=4500 ||key.codes[0]>=4500)
        {
    ...
    private boolean isAmharicRootLetter(Key key) {
        return((key.label.hashCode()%8==5)?true:false);
    }
    private boolean isAmharicRange(Key key)
    {
        return ((key.label.hashCode()>=4608 && key.label.hashCode()<=4988)?true:false);
    }
